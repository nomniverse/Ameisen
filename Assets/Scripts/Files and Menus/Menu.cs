...
public class Menu : MonoBehaviour {
	...
	void OnGUI() {
		...
		if (settings) {
			...
			if (GUI.Button (new Rect ((Screen.width / 2) - 50, Screen.height, 100, 50), "Keybindings")) {
					keys = true;
					settings = false;
			}
			//Creates a button to enter the sound menu
			if (GUI.Button (new Rect ((Screen.width / 2) - 50, (Screen.height / 2 + 25), 100, 50), "Audio Settings")) {
					sound = true;
					settings = false;
			}
			//Creates a button to enter the video menu
			if (GUI.Button (new Rect ((Screen.width / 2) + 50, 50, 100, 50), "Video Settings")) {
					video = true;
					settings = false;
			}
			//Creates a back button
			if (GUI.Button (new Rect ((Screen.width / 2) - 150, 50, 100, 50), "Back")) {
					switch (baseMenu) {
					case "Main":
							main = true;
							settings = false;
		...
		if (sound) {
			...
			GUI.Label (new Rect((Screen.width / 2) - 25, (Screen.height / 2) + 50, 50, 25), "Music Volume");
			musicVolume = GUI.HorizontalSlider (new Rect ((Screen.width / 2) - 50, (Screen.height / 2) + 5, 100, 10), 50, 0, 100);

			//Creates a horizontal slider for SFX volume
			GUI.Label (new Rect((Screen.width / 2) - 25, (Screen.height / 2) + 25, 50, 25), "Brightness");
			sfxVolume = GUI.HorizontalSlider (new Rect ((Screen.width / 2) - 50, (Screen.height / 2) - 10, 100, 10), 50, 0, 100);

			//Creates a save button for the sound options
			if (GUI.Button (new Rect ((Screen.width / 2) + 50, 50, 100, 50), "Save")) {
		...
		if (video) {
			...
			GUI.Label(new Rect((Screen.width/2) - 75, (Screen.height/2) + 50, 150, 25), "Useless FoV sldier (Impresses critics)");
			musicVolume = GUI.HorizontalSlider(new Rect((Screen.width/2) - 50, (Screen.height/2) + 5, 100, 10), 50, 0, 100);
			
			//Creates a horizontal slider for SFX volume
			GUI.Label(new Rect((Screen.width/2) - 25, (Screen.height/2) + 25, 50, 25), "SFX Volume");
			sfxVolume = GUI.HorizontalSlider(new Rect((Screen.width/2) - 50, (Screen.height/2) - 10, 100, 10), 50, 0, 100);
